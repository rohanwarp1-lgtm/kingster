/*
Author       : Dreamguys
Template Name: Kingster - Bootstrap Template
*/
(function ($) {
	"use strict";
...
if ($('.home-counter').length > 0) {
	...
	$('.home-counter').addClass('animated fadeInDownBig');
}
...
$(document).ready(function () {
    ...
    $('#serialNumber').on('input', function () {
        $('#error-serialNumber').text('');
    });
    $('#purchaseDate').on('change', function () {
        $('#error-purchaseDate').text('');
    });
    $('#checkSerialNumber, #checkMobileNumber').on('input', function () {
        const checkSerialNumber = $('#checkSerialNumber').val().trim();
        const checkMobileNumber = $('#checkMobileNumber').val().trim();

        if (checkSerialNumber !== '' || checkMobileNumber !== '') {
            $('#error-checkSerialNumber, #error-checkMobileNumber').text('');
        }
    });

    $('#purchaseSource').on('change', function () {
        let selectedOption = $(this).val();
        if (selectedOption == 'Online') {
            $('#address').addClass('d-none');
            $('#addressSelect').removeClass('d-none');
    ...
    $('#warrantyFormSubmit').click(function () {
        ...
        const purchaseSource = $('#purchaseSource').val();
        ...
        if (buyerName === '') {
            $('#error-buyerName').text('This field is required.');
            hasError = true;
        }

        if (mobile === '') {
            $('#error-mobile').text('This field is required.');
            hasError = true;
        }

        if (!purchaseSource) {
            $('#error-purchaseSource').text('This field is required.');
            hasError = true;
        }

        if (productName === '') {
            $('#error-productName').text('This field is required.');
            hasError = true;
        }

        if (serialNumber === '') {
            $('#error-serialNumber').text('This field is required.');
            hasError = true;
        }

        if (purchaseDate === '') {
            $('#error-purchaseDate').text('This field is required.');
            hasError = true;
        }
        ...
        if (!hasError) {
            let formData = {
                ...
                mobile: $("#mobile").val(),
                email: $("#email").val(),
                purchase_source: $("#purchaseSource").val(),
                address: $("#address").val(),
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response['status'] == 400){
                        $('#duplicate_msg').html('').html(response['message']);
    ...
    $('#checkStatusFormSubmit').click(function () {
        ...
        if (checkSerialNumber === '' && checkMobileNumber === '') {
            const errorMsg = 'Enter either product serial number or mobile number!';
            $('#error-checkSerialNumber').text(errorMsg);
            $('#error-checkMobileNumber').text(errorMsg);
            hasError = true;
        }
        ...
        if (!hasError) {
            ...
            let formData = {
                serial_number: $("#checkSerialNumber").val(),
                mobile_number: $("#checkMobileNumber").val()
            };
            var storeWarrantyAJAXUrl = $('#checkStatusFormSubmit').data('url');
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response['status'] === 4333) {
                        $('#warrantyResultContainer').html(`
                            <div class="alert alert-warning text-center">No warranty records found.</div>
                        `);
                        return;
                    }
                    ...
                    records.forEach(function (record) {
                        let $template = $('.warranty-tile-template').clone();
                        $template.removeClass('d-none warranty-tile-template');

                        $template.removeClass('warranty-not-found warranty-active warranty-expired warranty-rejected warranty-pending');

                        switch (record.warranty_status) {
                            case 'Active':
                                $template.addClass('warranty-active');
                                $template.find('.receiptHeading').text('Warranty Active!');
                                break;
                            case 'Expired':
                                $template.addClass('warranty-expired');
                                $template.find('.receiptHeading').text('Warranty Expired!');
                                break;
                            case 'Pending':
                                $template.addClass('warranty-pending');
                                $template.find('.receiptHeading').text('Warranty Pending!');
                                break;
                            case 'Rejected':
                                $template.addClass('warranty-rejected');
                                $template.find('.receiptHeading').text('Warranty Rejected!');
                                $template.find('.not-found-msg').text("Unfortunately, your warranty application has been rejected. Contact support at +91 88665 13744.");
                                break;
                            default:
                                $template.addClass('warranty-not-found');
                                $template.find('.receiptHeading').text('Warranty Not Found!');
                                break;
                        }

                        // Fill data
                        $template.find('.buyerName').text(record.buyer_name);
                        $template.find('.mobileNumber').text(record.mobile_number);
                        $template.find('.Email').text(record.email);
                        $template.find('.purchaseSource').text(record.purchase_source);
                        $template.find('.productName').text(record.product_name);
                        $template.find('.productSerialNo').text(record.serial_number);
                        $template.find('.purchaseDate').text(record.purchase_date);
                        $template.find('.expiryDate').text(record.expiry_date);

                        // Append to container
                        $('#warrantyResultContainer').append($template);
                    });

                    // Scroll to results
                    $('html, body').animate({
                        scrollTop: $('#warrantyResultContainer').offset().top
                    }, 800);
                }
        ...
        function showNotFound() {
            ...
            $('.receiptDesc').addClass('d-none');

            let $tile = $('#warrantyTile');
            $tile.removeClass('d-none warranty-active warranty-expired').addClass('warranty-not-found');
            $('#receiptHeading').text('Record Not Found!');
            $('.receiptDesc span').text('');
            $('.not-found-msg').text('Record not found. Please check the entered serial number.');

            $('html, body').animate({
                scrollTop: $tile.offset().top + $tile.outerHeight()
            }, 800);
        }
    });

    function applyStatusStyle($tile, status) {
        $tile.removeClass('warranty-active warranty-expired warranty-rejected warranty-pending warranty-not-found');

        if (status === 'Active') {
            $('#receiptHeading').text('Warranty Active!');
            $tile.addClass('warranty-active');
        } else if (status === 'Expired') {
            $('#receiptHeading').text('Warranty Expired!');
            $tile.addClass('warranty-expired');
        } else if (status === 'Pending') {
            $('#receiptHeading').text('Warranty Application Pending!');
            $tile.addClass('warranty-pending');
        } else if (status === 'Rejected') {
            $('#receiptHeading').text('Warranty Rejected!');
            $('.receiptDesc').addClass('d-none');
            $tile.addClass('warranty-rejected');
            $('.not-found-msg').text("Unfortunately, your warranty application has been rejected. Contact support: +91 88665 13744");
        } else {
            $('#receiptHeading').text('Warranty Not Found!');
            $tile.addClass('warranty-not-found');
        }
    }
    ...
    $('#toggleLoginPassword').on('click', function() {
        ...
        const icon = $('#toggleLoginPasswordIcon');
    ...
    $('form').on('submit', function(e) {
        ...
        let hasError = false;
        $('.error-message, .error-login').text('');
        ...
        if (!hasError) {
            ...
            var loginProcessURL = $('#loginFormSubmit').data('url');
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response['status'] == 400){
                        $('#error-login').html('').html("Please enter valid credentials!!");
                    }
