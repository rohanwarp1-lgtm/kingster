var jsonDataUrl = '../admin_assets/json/email-templates.json';

fetch(jsonDataUrl)
    .then(response => response.json())
    .then(data => {

        if ($.fn.DataTable.isDataTable('#email-templates-data')) {
            $('#email-templates-data').DataTable().destroy();
        }
        var table = $('#email-templates-data').DataTable({
            ordering: true,
            data: data,
            columns: [
                { data: 'ReferenceID' },
