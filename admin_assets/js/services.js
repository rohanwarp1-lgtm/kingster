var jsonDataUrl = '../admin_assets/json/services.json';
...
fetch(jsonDataUrl)
    ...
    .then(data => {
        ...
        if ($.fn.DataTable.isDataTable('#services-data')) {
            $('#services-data').DataTable().destroy();
        }
        var table = $('#services-data').DataTable({
            ordering: true,
            data: data,
            columns: [
                { data: '#' },
