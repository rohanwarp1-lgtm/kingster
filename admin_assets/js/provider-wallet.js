var jsonDataUrl = '../admin_assets/json/provider-wallet.json';
...
fetch(jsonDataUrl)
    ...
    .then(data => {
        ...
        if ($.fn.DataTable.isDataTable('#provider-wallet-data')) {
            $('#provider-wallet-data').DataTable().destroy();
        }
        var table = $('#provider-wallet-data').DataTable({
            ordering: true,
            data: data,
            columns: [
                { data: '#' },
