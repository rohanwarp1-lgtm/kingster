var jsonDataUrl = '../admin_assets/json/system-backup.json';

fetch(jsonDataUrl)
    .then(response => response.json())
    .then(data => {
        if ($.fn.DataTable.isDataTable('#system-backup-data')) {
            $('#system-backup-data').DataTable().destroy();
        }
        var table = $('#system-backup-data').DataTable({
            ordering: true,
            data: data,
            columns: [
                { data: 'name' },
